...
<div class="">
  ...
  <form id="frmManagePaymentMethod" class="frm-credit-card-input" role="form" method="post" action="{if $editMode}{routePath('account-paymentmethods-save', $payMethod->id)}{else}{routePath('account-paymentmethods-add')}{/if}">
    ...
    <div class="alert-container">
      <div class="alert alert-warning gateway-errors assisted-cc-input-feedback w-hidden">
        <div>
          {* alert-icon *}
          <div class="alert-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-frown-icon lucide-frown">
          ...
          <div class="alert-content">
            <p class="alert-message">{lang key='paymentMethodsManage.invalidCardDetails'}</p>
          </div>
          {* close-alert *}
      ...
      <div class="col-md-8">
        <div class="d-flex flex-wrap gap-4">
          {if $enabledTypes['tokenGateways']}
            {foreach $tokenGateways as $tokenGateway}
              <label class="form-check">
                <input type="radio" class="form-check-input icheck-button" name="type" value="token_{$tokenGateway}" data-tokenised="true" data-gateway="{$tokenGateway}" {if $editMode && $payMethod->isCreditCard() && $payMethod->gateway_name == $tokenGateway} checked{/if}{if $editMode} disabled{/if}>
          ...
          {if $enabledTypes['localCreditCard']}
            <label class="form-check">
              <input type="radio" class="form-check-input icheck-button" name="type" value="localcard" {if ($editMode && $payMethod->isCreditCard() && !$payMethod->isTokenised()) || (!$editMode && $paymentMethodType != 'bankacct')} checked{/if}{if $editMode} disabled{/if}>
              <span class="checkmark"></span>
          ...
          {if $enabledTypes['bankAccount']}
            <label class="form-check">
              <input type="radio" class="form-check-input icheck-button" name="type" value="bankacct" {if ($editMode && !$payMethod->isCreditCard()) || ($paymentMethodType == 'bankacct')} checked{/if}{if $editMode} disabled{/if}>
              <span class="checkmark"></span>
    ...
    {* fieldgroup *}
    <div class="fieldgroup-loading w-hidden mb-4">
    ...
    <div class="fieldgroup-auxfields{if $remoteUpdate} w-hidden{/if}">
      <div class="form-group row mb-8">
        ...
        <div class="col-md-8">
          ...
          <br>
          <a href="#" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#modalBillingAddress">
      ...
      <div class="form-group row submit-container">
        <div class="col-md-8 offset-sm-4 d-flex col-gap-2">
          <button type="submit" name="submit" id="btnSubmit" class="btn btn-light">{lang key='clientareasavechanges'}</button>
          <a href="{routePath('account-paymentmethods')}" class="btn btn-ghost-light">{lang key='cancel'}</a>
        </div>
      </div>
    </div>
    {* inputs *}
