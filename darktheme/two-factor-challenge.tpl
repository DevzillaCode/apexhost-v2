<div class="card mw-540 mb-md-4 mt-md-4">
    <div class="card-body px-sm-5 py-5">
        ...
        {if $newbackupcode}
            {include file="$template/includes/alert.tpl" type="success" message="{lang key='twofabackupcodereset'}"}
        {elseif $incorrect}
            {include file="$template/includes/alert.tpl" type="error" message="{lang key='twofa2ndfactorincorrect'}"}
        {elseif $error}
            {include file="$template/includes/alert.tpl" type="error" msg=$error}
        {else}
            {include file="$template/includes/alert.tpl" type="warning" message="{lang key='twofa2ndfactorreq'}"}
        {/if}
        ...
        <form method="post" action="{routePath('login-two-factor-challenge-backup-verify')}" id="frmTwoFactorBackup"{if !$usingBackup} class="w-hidden"{/if}>
            ...
            <p class="text-center">
                <a href="#" class="btn btn-light btn-block mt-2" id="backupCodeCancel">
                    {lang key='cancel'}
                </a>
            </p>
        </form>
