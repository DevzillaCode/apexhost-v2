{foreach $renewableItems as $renewableItem}
  <div class="service-renewal" data-product-name="{$renewableItem.product->name}" data-service-id="{$renewableItem.serviceId}" data-service-domain="{$renewableItem.domain}" {if $renewableItem.renewable === false}style="display: none;" data-is-renewable="false" {else}data-is-renewable="true" {/if}>
    ...
    <h3 class="text-size-xl font-weight-semibold mb-4">
      {$renewableItem.product->name}
    </h3>
    <h4 class="text-size-lg mb-2">
    ...
    {if !empty($renewableItem.addons)}
      <div class="addon-renewals" {if $renewableItem.renewableCount <= 0}style="display: none;" data-is-renewable="false" {else}data-is-renewable="true" {/if}>
        <h4 class="text-size-lg mb-2">Addons</h4>
        <div>
